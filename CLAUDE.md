...
> **UPDATED AT:** 2026-06-22 (REPO CLEANUP PASS — on `main`. Removed the **Muse Observatory** work (migrated to its own standalone repo) and ran a behavior-preserving cleanup: deleted the 7 unused colored muse PNGs (`muse/{lunes,ares,rabu,thunor,shukra,dosei,solis}.png` — the orbit/popup use the `-white` variants, built dynamically); deleted the transient root working-notes (`memo-edits-1906.md`, `memo-orbit-edit.md`, `memoedit-comettransition.md`, `memoedit-comettransition-history.md`); moved `to-do.md` → `docs/notes/to-do.md`; unexported the internal-only `getFocusable` in `src/ui/focus-trap.js`; removed 5 unreferenced design tokens (`--font-h2-size`, `--font-h2-height`, `--intro-logo-size`, `--z-events`, `--z-popup`); and consolidated the duplicated DPR-cap formula in `src/ui/process-links.js` + `src/ui/muse-galaxy.js` to import the canonical `DPR()` from `src/webgl/gl-context.js` (`intro.js` keeps its own intentionally-different uncapped-on-desktop DPR). Build green, no behavior/visual change. PRIOR — SAFARI / CROSS-BROWSER FIX PASS. All device-confirmed on real iOS Safari. **iOS muse-logo timing drift (Issue 2a):** `vhToPx` (`src/scroll/timeline.js`) now converts against the **CSS `vh` unit measured via a hidden `100vh` probe** (`measureVhUnit`, re-run inside `applyHeightsToCss`) instead of `window.innerHeight` — iOS CSS `vh` = the *large/stable* viewport (URL-bar retracted, never changes) while `innerHeight` = the *dynamic* one, so the section heights (written in CSS vh) and the scroll math now share ONE unit; this killed the ~196px drift that switched the muse logo too early (`?debug` panel Δ 196→0). **Muse-logo Safari scrub-lag (Issue 2b/6):** a blurred CSS `drop-shadow` re-rasterises on EVERY opacity-scrub frame on iOS *even with `translateZ(0)` layer promotion* (confirmed on device — promotion did NOT fix it), so the logo's fade trailed the scroll. The soft glow is now **BAKED INTO new PNG assets** `muse_logo_white_glow.png` / `muse_logo_black_glow.png` (PIL bake from `muse_logo_black.png`: the logo fills its frame edge-to-edge so the halo needs room → expanded **symmetric-padded canvas at factor 1.25**; white asset is genuinely white, replacing the old `brightness(0) invert(1)` recolour; downsampled to 1600px). `.muse-logo-white` / `.muse-logo-black` are now **`filter: none`** (+ `translateZ(0)`), and `.muse-logo-image { width: calc(var(--muse-logo-size) * 1.25) }` compensates the padding so the glyph still renders at `--muse-logo-size`. The glow is intentionally soft (≈invisible on a phone; left as-is per product call — re-bake with a higher alpha to strengthen). **Constellation empty on full scroll-up (Issue 3):** `setIntroVisible(true)` now repaints `drawExplosion(1)` when `explosionStarted` — the explosion scrub only runs within `intro.explosion` (192–292vh) and intro-exit cleared the 2D canvas, so scrolling ALL the way up from muse showed the statement/smoke over an empty canvas until the scrub repainted; the repaint restores the settled frame. **Popup symbol outline on first open (Issue 5):** `MusePopup.setMuses` now **preloads + `img.decode()`s all 7 symbols** (keeping refs) so even the FIRST open is clean (Safari painted a freshly-set, async-decoded PNG with a brief edge/outline artifact). **Muse intro top-heavy on mobile (Issue 1):** at ≤768px `.muse-intro-text-top { top: 21% }` / `.muse-intro-text-bottom { bottom: 9% }` (`responsive.css`; desktop 16%/16% in `muse.css` untouched). **Orbit froze while scrolling (Issue 4):** REMOVED the `touchstart → orbitPauseUntil += 2s` in `MuseScroll.attachHandlers` — with Lenis `syncTouch` off, iOS touch scroll is native and iOS throttles/pauses rAF during a momentum fling, so the orbit (the whole `Renderer` loop) naturally stills WHILE you scroll and resumes the instant you stop; the explicit pause only delayed the resume. **Diagnostic scaffolding KEPT (inert without `?debug`):** `src/dev/viewport-debug.js` (imported in `main.js` `boot`; live overlay of panel Δ / CSS-vh unit / rAF frames / scroll / muse offsets) + a `Renderer.frames` rAF tick counter (`src/webgl/renderer.js`, telemetry for when iOS pauses rAF). PRIOR — COPY/WEBSITE-REWRITE BRANCH: full copy rewrite + UI refinement rounds. **Canon fixes:** muse hexes corrected — **Shukra `#7F49A2`** (lighter), **Dosei `#5E47A1`** (darker); **Ares cause = "Rewilding"** (not "Reforestation"). cocoex is described WITHOUT "DAO/blockchain/on-chain" now — a **non-profit transdisciplinary social-impact lab**; banned tone words now also include DAO / blockchain / on-chain. **Muse orbit halo** recoloured from the rainbow to **each muse's OWN hue** (`.muse-orbit-card::before` — dim same-hue ring + a brighter white-lifted head circling via `--beam-angle`). **Muse popup galaxy** → immersive **"deep galaxy"** (`muse-galaxy.js`, ~140 particles, full-screen inflow centred on the disc via `maxR` = farthest-screen-corner + per-particle **depth/parallax** + a slow global turn; dropped the portrait stretch). **Muse popup entrance/exit** = **disc-led bloom** (disc `0.62→1` `back.out`, then title→cause→text stagger; exit releases the disc to `0.84`; added a real reduced-motion crossfade branch). **Intro Beat-2 statement** "Art / Community / Impact" → **3 bold STACKED PILLARS igniting one-by-one** (glow-bloom on a tweenable `--glow`), still leaving WITH the constellation smoke (unified fade-out window unchanged). **Comet toggle** pill: `.beam-glow` **racing-rainbow rim** + a **"liquid glass" body** (`.pill-glass` child — heavily-muted rainbow drifting behind a frosted, grainy privacy-glass; no `backdrop-filter`) + the thumb is **draggable** (`Toggle._initDrag`, Pointer-Events on `.comet-pill`, snap-to-nearest on release; tap still switches; `touch-action: pan-y`). **Campaign CTAs** relabeled **"Explore campaigns"** (arrow removed, centred) carrying the same `.beam-glow` rim + a `.cta-glass` frosted glass on hover (shares `.pill-glass` styles). Footer gained a mission line + legal identifier. Mobile: comet step title→description gap tightened. Living branch notes in `docs/website-rewrite-plan.md`. PRIOR: COMET DIVE — REVEAL-START FLASH FIX. The Comet→toggle "dive" reveals the Stardust/Horizon panel THROUGH the growing Comet-Collab swirl — a 2D-canvas swirl (`src/ui/comet-dive.js`) + WebGL ripple (`src/webgl/burst-ripple.js`), both drawn by the central `Renderer` from one `dive.p`, masking the static methods panel with the holey swirl SVG (`comet-mark-white.svg`) so it reveals ONLY within the WHITE of the logo (content + section masked-revealed through the swirl). The flicker was the section **background** flashing **unmasked** ("the background turns white") for one scroll step at the reveal start: `comet.js` sets the panel opaque (`.set(methods,{opacity:1},70)`) but `comet-dive.js` applied the mask only when `reveal > 0`, so at the `reveal == 0` boundary the panel was opaque-but-unmasked (CDP-confirmed `op:1, mask:no` at scroll 10430–10434, mobile). **Fixed** in `comet-dive.js` `draw()`: apply the mask for the whole reveal **including `reveal == 0`** (dropped the `reveal > 0` gate) so the panel is never opaque-but-unmasked; and `setState`'s out-of-band clear changed from `reveal<=0` to `appear<=0` (`reveal==0` is now the masked take-over, so clearing there would re-flash; `appear<=0` still releases on jumps before the dive). NOT the toggle content (an earlier content-gate attempt was reverted — gating it proved the flash persisted). Full feature history + knobs in `memoedit-comettransition.md` (`comet-dive.js`/`burst-ripple.js` are NOT yet in the module map below). ALSO this pass: **stars +10%** — the unified white-on-black starfield is `createStarfield('unified-starfield-canvas', { intensity: 0.275 })` (was the `0.25` default; `intensity` is the linear brightness mult in `STARFIELD_FRAG`). **Intro idle-dots fix** — the orbit `onUpdate` idle branch now ALSO clears the dots' inline `opacity`/position so `.intro-idle .orbit-dot{opacity:0}` re-hides them on scroll-back to the top (they'd been left stuck visible + off-position by `updateOrbit`'s inline `opacity:1`). **Mission copy replaced** (`#mission-overlay .reveal-text` top+bottom = the new "transdisciplinary social impact lab" text) + **`.reveal-text` sized down** from `--font-body-size` to `--font-lead-size`/`--font-lead-height` now that it's multi-sentence prose. PRIOR — MUSE PACING FIX + ORBIT HALO. **Critical:** `muse.js`/`comet.js` fed ScrollTrigger `top+=Xvh` strings, which GSAP reads as **px** — collapsing the whole muse timeline into ~460px (~½ a swipe), which is why editing `PHASES` "did nothing". `buildMuseTimeline` now converts vh→px via `phase().startFromSection()`/`endFromSection()` in function-form start/end (the intro pattern); the muse pacing is now actually driven by `PHASES`. **Muse 490vh** = `fadein 50 / hold 120 / switch 100 / orbitHold 220`: intro copy reveal **matched to the cocoex mission** (~50vh fade-in + ~120vh readable hold), and the orbit gets a **~120vh pinned hold** before its 100vh scroll-out (`orbitHold` = 100 structural exit + 120 dwell). **NEW roaming "click me" halo:** `MuseScroll` moves a spectrum comet-trail glow (`.muse-orbit-card::before`, `.beam-glow` recipe, continuous ~300° trail + bright leading head, `2s` spin) across one random orbit muse at a time (`.is-hinting`), gated to when the orbit is live, yielding to hover/popup. `PARTNERS` in `data.js` now carry real clickable `url`s. **comet.js still has the latent vh-as-px bug** (left per product call — comet pacing unchanged). Prior pass (RIPPLE + SAFARI): Intro **big-bang pulse → muse-spectrum RIPPLE**: `INTRO_FRAG` now draws concentric ease-OUT wavefront rings (decelerate as they widen, reach all screen edges), tinted across the 7 muse hues (`museTint`), organic via snoise wobble. `u_pulse` is **scroll-driven from the explosion progress** (`updateExplosion`: `pulse = min(1, progress/0.45)`), not time — so it rides the burst; the time-incrementer `Renderer` layer was removed. Statement (`#intro-statement`) now **exits WITH the constellation** (one timeline `explosion.start → mission-mid`, fade-out lands on the smoke window). Idle orbit dots fixed (`.intro-idle .orbit-dot { animation: none }` — pulseDot keyframes were overriding `opacity:0`, leaving dots pulsing at the top-left). Intro tagline flare is now **scroll-scrubbed** (`--sweep` on a `scrub` trigger) + a wider feathered band. Comet **starline wire now invisible** — only the muse-spectrum comet runs the (now hidden) 1→5 path; `ProcessLinks` gained a **self-heal resize** in `draw()` (iOS `100dvh` URL-bar fix). `.beam-glow::after` bloom **masked to the rounded ring** (Safari leaked a square box on the blurred filled pseudo). Muse popup subtitle opened up (`letter-spacing 0.14em`, larger) + subtitle/description grouped (`.muse-popup-caption`). Footer **~50% smaller desktop / ~20% mobile** (`min-width:1024px` breakpoint) + bottom buffer. Safari audit: guarded a shader NaN (`pow(max(sin·,0),0.6)`), added `color-mix` disc fallbacks. **Section totals: intro 592 + muse 490 + comet 500 = ~1582vh** (plus static events + footer). 5 WebGL + 3 2D-canvas surfaces.)
...
**cocoex** is a non-profit transdisciplinary social-impact lab blending art, community participation and social impact. (Note: do NOT describe it as a "DAO" or with "blockchain/on-chain" — those terms were removed in the copy rewrite; it uses technology to make every contribution traceable.)
...
- Tone: precise, poetic. Never: "DAO", "blockchain", "on-chain", "innovative", "transformative", "impactful", "leveraging".
...
| 2 | **Ares** | Rewilding | `#D54D2E` | `--ares` | Mars | Tuesday |
| 3 | **Rabu** | Human Rights | `#8CB07F` | `--rabu` | Mercury | Wednesday |
| 4 | **Thunor** | Renewable Energy | `#F8D86A` | `--thunor` | Jupiter | Thursday |
| 5 | **Shukra** | Bio-diversity | `#7F49A2` | `--shukra` | Venus | Friday |
| 6 | **Dosei** | Zero Hunger | `#5E47A1` | `--dosei` | Saturn | Saturday |
...
| Path | Responsibility |
|------|----------------|
| `src/main.js` | `boot()`: import CSS, `applyHeightsToCss()`, `initSmoothScroll()`, create 5 WebGL surfaces (+ the 2D popup galaxy) + register them as gated `Renderer` layers, init the four sections, `initSectionGate()`, `Renderer.start()`, debounced resize, `initViewportDebug()` (last — `?debug`-gated, inert otherwise). |
| `src/data.js` | `CONFIG`, `isMobile`, `DOT_COLORS`, `CONSTELLATION_REF`, `CONNECTIONS`, `MUSES`, `PARTNERS`, easing fns. |
| `src/scroll/timeline.js` | **Single source of truth** for scroll pacing — `PHASES`, builder, `phase()`, `sectionSpan()`, `vhToPx()`, `applyHeightsToCss()`, `COMET_CONST_HIDE_VH`. `vhToPx` converts against the **measured CSS `vh` unit** (`measureVhUnit`, a hidden `100vh` probe re-run in `applyHeightsToCss`), NOT `window.innerHeight` — iOS CSS vh = stable large viewport ≠ dynamic innerHeight (see Common Bugs). |
...
| `src/ui/muse-galaxy.js` | `createMuseGalaxy(canvasId)` — 2D **"deep galaxy"** particle field for the popup (~140 particles, polar `(r,θ)`). Fills the viewport — `maxR` = the farthest screen-corner from the disc, centred on the live card rect — so particles **stream in from every edge** and spiral into the disc rim (absorbed, never over the face). Each carries a **depth** `d∈[0,1]` (near = big/bright/fast/long-streak, far = tiny/dim/slow) and the whole field has a slow **global turn** (`globalTheta`) → volumetric parallax. Drawn by the gated `Renderer` only while the popup is open. (Was ~120 dots with a portrait `sy` stretch — the stretch is gone.) |
...
| `src/styles/comet.css` | `.comet-panel-intro` literal `200vh` / `.comet-panel-tabs` literal `300vh`; pill toggle (`.comet-pill.beam-glow` + `.pill-glass` liquid-glass body + draggable thumb, `:focus-visible`); `.comet-cta` "Explore campaigns" buttons (`.beam-glow` rim + `.cta-glass` hover glass, shares `.pill-glass`); `@keyframes fadeInPanel`. |
...
| `src/styles/responsive.css` | 1024/768/480 breakpoints + `max-height:500px`; reduced-motion lives in `base.css`. At ≤768px the muse intro copy block is shifted down (`.muse-intro-text-top { top:21% }` / `.muse-intro-text-bottom { bottom:9% }`) so it isn't top-heavy on tall phones. |
| `src/dev/viewport-debug.js` | `initViewportDebug()` — **`?debug`-gated** on-device diagnostic overlay (inert otherwise). Live rows: muse `panel Δ` (CSS-vh height vs `innerHeight`-based assumption), CSS-vh unit, `Renderer.frames`, scroll, muse offsets. Built for the iOS-Safari muse-logo timing drift (Issue 2); KEEP as reusable device-debug scaffolding. |
...
- **`intro.explosion` (192–292vh):** 7 colored constellation dots explode from center on the 2D `#constellation-canvas`, and the WebGL `#bg-canvas` fires a **muse-spectrum ripple** synced to the same burst — `u_pulse` is driven straight from the explosion scroll progress (`updateExplosion`: `pulse = min(1, progress/0.45)`, NOT time), so the concentric ease-out wavefront expands + decelerates with the dots flying out. Both front-loaded so they settle then hold. The layout reference is landscape (1400×800); on a **portrait** viewport (`height > width`) `initFireworkDots` transposes it 90° (ref-x → vertical, ref-y → horizontal). `resize()` re-projects if the explosion already ran. **Beat 2 — statement** (`#intro-statement` = "Art / Community / Impact", now **3 bold STACKED PILLARS** — `.intro-statement-word` spans) **ignites one-by-one** top→bottom over the first ~40% of one timeline spanning `explosion.start → mission-mid` (a glow that blooms then settles to a faint halo, riding a tweenable `--glow` custom prop), holds over the settled constellation, then **fades out together with the constellation** as one block (the unified fade-out lands exactly on the smoke-clear window, so words + dots leave as one). Reduced-motion → plain staggered fade.
...
- **Switch (170–270vh):** the white `.muse-section` bg fades in, the center logo **opacity-crossfades white→black** (two stacked `<img>`: `#muse-logo-white` out, `#muse-logo-black` in — NO `filter()` tween), and the intro copy fades out as the orbit takes over. The logos are the **baked-glow PNGs** (`muse_logo_white_glow.png` / `muse_logo_black_glow.png`, `filter: none`) — the soft halo is pre-rendered into the asset because a live blurred `drop-shadow` lagged the opacity scrub on iOS Safari; `.muse-logo-image` width is `× 1.25` to offset the assets' transparent halo padding (see UPDATED AT + Common Bugs).
...
**Roaming "click me" halo:** to signal the discs are clickable, `MuseScroll` moves a halo from muse to muse. `.muse-orbit-card::before` is a conic-gradient in **each muse's OWN colour** (`var(--muse-color)`) — a dim same-hue ring with a brighter, **white-lifted head** that travels as `--beam-angle` spins (`2s`), blurred so it blooms out behind the disc (lifted brighter than the disc body so it doesn't wash into the edge; solid `--muse-color` fallback for Safari <16.2). (Was the 7-hue rainbow — recoloured because the same spectrum on every coloured disc read as confusing.) Hidden until the owning `.muse-orbit-item` gets `.is-hinting`. The JS roamer (`scheduleHint`/`showHint`/`clearHint`) gives one random muse `.is-hinting` at a time, hopping every ~3.5s (CSS opacity crossfades the swap); it's gated to when the orbit is actually live (keys off `update()`'s `lastTime`) and yields to hover/`:focus-visible`/`MusePopup.isOpen`/hidden tab. Hover/focus calls `clearHint()` so the halo never fights the flip. Reduced-motion freezes the spin to a static glow.
...
Pill toggle (`.comet-pill.beam-glow`) — labels are just "Stardust" / "Horizon" (no "I·/II·"; the numbering lives in the panel subtitles). Carries the `.beam-glow` **racing-rainbow rim** PLUS a **"liquid glass" body**: a `.pill-glass` child (z-0, below the thumb + labels) holds a heavily-muted muse-spectrum rainbow drifting (`beam-spin 12s`) behind a frosted, grainy **privacy-glass** (`::before` = white-tint mute + sheen; `::after` = `feTurbulence` grain) — the colour reads softly *through* it rather than a flat body. No `backdrop-filter`. Switches between:
...
The toggle is wired by `Toggle.init()` attaching `click` listeners to `#tab-stardust` / `#tab-horizon` (**no inline onclick, no global `window.switchTab`**); `Toggle.switch(method)` flips `.active` classes + the `#pillSlider` position. The thumb is also **draggable** — `Toggle._initDrag` adds Pointer-Events on `.comet-pill` (`touch-action: pan-y`): a horizontal drag past a 4px threshold follows the finger 1:1 (`.pill-slider.dragging` drops the transition) and **snaps to the nearest side on release**; a tap still switches (a capture-phase click-swallow stops a drag's trailing click from double-switching). Switch smoothness: `.pill-slider` has `will-change: transform` (composites independent of the beam repaint), the active label's whiten is delayed `0.1s` so it changes as the slider arrives (no white-on-offwhite flash), and the focus ring is `:focus-visible` only (no mouse-click outline pop). No `backdrop-filter` on the pill — it re-blurred the moving bloom and stuttered. The two panels top-align (`align-content: start`) so headers don't jump between Stardust/Horizon. Panel body copy is `--font-lead-size`, left-aligned (was justified). Titles are solid (the **hollow/outline** treatment lives on the words "Stardust"/"Horizon" in the *intro* copy, not the panel titles).
...
--z-white-section: 30
```
The `--z-events` (40) and `--z-popup` (1000) tokens were removed in the 2026-06-22 cleanup — the events wrapper and muse popup already hardcode those z-index literals, so the tokens had zero references.
...
8. `initViewportDebug()` — `?debug`-gated on-device diagnostic overlay (inert without the query param).
...
- Orbit stability on mobile is **passive, not an explicit pause**: the old `touchstart → pause 2s` was REMOVED (it froze the orbit for the whole drag + 2s after every scroll touch — the "orbit stuck while scrolling" bug). With Lenis `syncTouch` off, iOS touch scroll is native and iOS pauses rAF during a momentum fling, so the orbit naturally stills while you scroll and resumes when you stop — a muse is already steady right as you reach to tap it. Do NOT re-add a `touchstart` pause.
...
├── muse/
│   ├── muse_logo_black.png                                                                  (source for the bake below — no longer referenced directly by the page)
│   ├── muse_logo_white_glow.png · muse_logo_black_glow.png                                  (the muse-stage center logo — soft GLOW BAKED IN, white asset is genuinely white; `filter:none` + `.muse-logo-image` width ×1.25 for the halo padding. PIL-baked from muse_logo_black.png)
│   ├── lunes-white.png · ares-white.png · rabu-white.png · thunor-white.png · shukra-white.png · dosei-white.png · solis-white.png   (white glyphs; orbit/popup render these on a dynamically-colored disc — the colored variants were removed in the 2026-06-22 cleanup)
...
**iOS CSS `vh` ≠ `window.innerHeight`:** on iOS Safari, CSS `vh`/`dvh` heights resolve to the **large/stable** viewport (URL bar retracted) and never change as the bar shows/hides, while `window.innerHeight` is the **dynamic** viewport (shrinks under the bar). Section heights are written in CSS `vh` (`applyHeightsToCss`), so any scroll math that uses `innerHeight` is short by the URL-bar ratio and the error COMPOUNDS with depth (~196px adrift by the muse panel → the muse logo switched too early). `vhToPx` therefore measures the real CSS vh unit via a hidden `100vh` probe (`measureVhUnit`, re-run in `applyHeightsToCss`) — **never go back to `innerHeight` for scroll-layout math.** Diagnose with the `?debug` overlay (`src/dev/viewport-debug.js`): `panel Δ` ≠ 0 = this. Fixed 2026-06-19.

**Safari: blurred CSS `filter` + scrubbed opacity = lag (bake it):** an element with a blurred `filter`/`drop-shadow` whose **opacity is scroll-scrubbed** re-rasterises the filter EVERY frame on iOS Safari and trails the scroll — and `transform: translateZ(0)` layer promotion does **NOT** reliably fix it (tried on the muse logo, still lagged on device). The robust fix is to remove the live filter: **bake the effect into the image asset** (`filter: none`) — that's why the muse-stage logos are the pre-baked `*_glow.png` files. Chrome caches the filtered layer so it looks fine there — easy to misdiagnose as a timing bug.
