...
//
//   dev   : serve /observatory/campaigns.json + /observatory/<slug>/ from memory
//           (drafts included for preview); reload when content/ or record.css change.
//   build : emit dist/observatory/campaigns.json + dist/observatory/<slug>/index.html
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (!url.startsWith('/observatory')) return next();
        ...
        if (url === '/observatory/campaigns.json') {
          res.setHeader('Content-Type', 'application/json');
          res.end(out.campaignsJson);
          return;
        }

        // /observatory/<slug>(/)(index.html)
        const m = url.match(/^\/observatory\/([^/]+)\/?(?:index\.html)?$/);
        const page = m && out.pages.find((p) => p.slug === m[1]);
        if (page) {
    ...
    async closeBundle() {
      ...
      const base = path.join(outDir, 'observatory');
      await fs.mkdir(base, { recursive: true });
      await fs.writeFile(path.join(base, 'campaigns.json'), out.campaignsJson);
